	...
	<view class="sunui-uploader-bd">
		<view class="sunui-uploader-files">
			...
			<!-- <image class="sunui-uploader-img" :style="upload_img_wh" :src="item.path" mode="aspectFill" />
					<image v-if="isDelShow" class="sunui-img-removeicon right" @click.stop="removeImage(index)" v-show="upimg_move && oss"
					 :src="oss+'icon_img_canel.png'"></image>
			...
			<view v-show="upload_len < upload_count" hover-class="sunui-uploader-hover" class="sunui-uploader-inputbox" @click="chooseImage"
			 :style="upload_img_wh">
				<image v-if="oss" :src="oss+'bg_add_img.png'" style="width: 88rpx;height: 88rpx;"></image>
			</view> -->
			<!-- <block v-for="(item, index) in upload_before_list" :key="index"> -->
			<view class="sunui-uploader-file" :class="[item.upload_percent < 100 ? 'sunui-uploader-file-status' : '']" @click="chooseImage">
				<!-- step1.这里修改服务器返回字段 ！！！ -->
				<image class="sunui-uploader-img" :style="upload_img_wh" :src="imgAvatar" mode="aspectFill" />
				<image v-if="isDelShow" class="sunui-img-removeicon right" @click.stop="removeImage(index)" v-show="upimg_move && oss"
				 :src="oss+'icon_img_canel.png'"></image>
				<view class="sunui-loader-filecontent" v-if="item.upload_percent < 100">{{ item.upload_percent }}%</view>
			</view>
...
<script>
	export default {
		...
		props: {
			...
			url: {
				type: String,
				default: 'https://api.woneng.net/admin/file/app/sftp/upload',
				// default: 'http://183.131.134.242:10026/admin/file/app/sftp/upload'
			},
			// 上传样式宽高
			upload_img_wh: {
				type: String,
				default: 'width:204rpx;height:204rpx;border-radius:20rpx;margin-right: 29rpx;margin-top: 30rpx;'
			},
			// 上传数量
			upload_count: {
				type: [Number, String],
		...
		// 		// this.imgAvatar = res.data.data.avatar;
		// 		console.log('aaa', res)
		// 	});
		// },
			...
			upImage(paths, header) {
				...
				Promise.all(promises)
					.then(function(data) {
						...
						_self.imgAvatar = data[0]

						console.log('data', _self.imgAvatar)
					})
					.catch(function(res) {
